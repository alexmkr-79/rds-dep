...
export function Footer() {
  return (
    <footer className="relative mt-32 border-t border-border/60 bg-[oklch(0.10_0.014_50)]">
      ...
      <Reveal stagger={0.08} className="container-page py-16 grid gap-12 lg:grid-cols-[1.4fr_1fr_1fr_1fr]">
        ...
        <RevealItem variant="fade-up">
          <h4 className="font-display text-cream/90 text-sm tracking-[0.28em] uppercase mb-5">Connect</h4>
...
function FooterColumn({
  ...
  return (
    <div>
      <h4 className="font-display text-cream/90 text-sm tracking-[0.28em] uppercase mb-5">{title}</h4>
      <ul className="space-y-2.5 text-sm">
        {links.map((l) => (
          <li key={l.label}>
            <Link to={l.to} hash={l.hash} className="text-cream/70 hover:text-copper-light transition-colors story-link">
              {l.label}
            </Link>
          </li>
        ))}
...
const SOCIALS = [
  { label: "Instagram", href: "https://instagram.com/", svg: SVG('<rect x="3" y="3" width="18" height="18" rx="5"/><circle cx="12" cy="12" r="4"/><circle cx="17.5" cy="6.5" r="0.6" fill="currentColor"/>') },
  { label: "Facebook", href: "https://facebook.com/", svg: SVG('<path d="M17 2h-3a4 4 0 0 0-4 4v3H7v4h3v9h4v-9h3l1-4h-4V6a1 1 0 0 1 1-1h3z"/>') },
  { label: "YouTube", href: "https://youtube.com/", svg: SVG('<rect x="2" y="5" width="20" height="14" rx="3"/><path d="M10 9l5 3-5 3z" fill="currentColor"/>') },
  { label: "TikTok", href: "https://tiktok.com/", svg: SVG('<path d="M15 3v9a4 4 0 1 1-4-4"/><path d="M15 3a5 5 0 0 0 5 5"/>') },
  { label: "Google Maps", href: "https://maps.google.com/?q=Rachnakar+Design+Studio+Baner+Pune", svg: SVG('<path d="M12 22s-7-7.58-7-12a7 7 0 0 1 14 0c0 4.42-7 12-7 12z"/><circle cx="12" cy="10" r="2.5"/>') },
];