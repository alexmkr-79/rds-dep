...
const services = [
  { title: "CNC Routing & Cutting", desc: "Precision cuts on wood, MDF, plywood and acrylic — repeatable to the millimetre." },
  { title: "3D Carving & Engraving", desc: "Sculptural relief work, bas-reliefs, name plates and bespoke wall art." },
  { title: "Temple & Jali Design", desc: "Devotional mandirs and intricate jali screens with traditional motifs." },
  { title: "Custom Furniture Panels", desc: "Designer doors, wardrobe shutters, partitions and feature wall panels." },
];
...
function Home() {
  ...
  return (
    <>
      {/* HERO */}
      <section ref={heroRef} className="relative min-h-[100svh] flex items-end pt-20 pb-16 sm:pb-24 overflow-hidden">
        <HeroVideo />
        <div className="container-page relative z-10 grid gap-8" data-parallax="14">
          <span data-hero-eyebrow className="text-[0.7rem] sm:text-xs tracking-[0.42em] uppercase text-copper-light/85">
          ...
          <h1 className="font-display text-balance text-[clamp(2.75rem,8vw,7rem)] leading-[0.95] text-cream max-w-5xl">
            <span data-hero-devnag className="font-devnag block text-copper-light text-[clamp(3rem,9vw,8rem)] leading-[1] mb-3">
              रचनाकार
            </span>
            <span data-hero-line className="block">Where heritage carving meets</span>
            <span data-hero-line className="block"><em className="italic text-copper-light">modern precision.</em></span>
          </h1>
          <p data-hero-para className="max-w-xl text-cream/75 text-base sm:text-lg leading-relaxed">
            A premium CNC design studio in Baner, Pune — crafting bespoke doors, temples, jali panels and
            sculptural furniture for discerning homes and architects.
          </p>
      ...
      <section id="services" className="container-page py-24 sm:py-32">
        ...
        <Reveal stagger={0.1} className="mt-14 grid gap-px bg-border/40 rounded-3xl overflow-hidden border border-border/40">
          {services.map((s) => (
      ...
      <section id="catalogue" className="container-page py-24 sm:py-32 border-t border-border/40">
        <Reveal variant="fade-up" className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-6">
          <SectionLead eyebrow="Catalogue" title="15 categories. 200+ CNC-ready designs." />
          <Link to="/products" className="text-sm text-copper-light hover:text-cream tracking-wider uppercase story-link">
            View all →
          </Link>
        </Reveal>
        <Reveal stagger={0.05} className="mt-12 grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3 sm:gap-4">
          {CATEGORIES.slice(0, 10).map((c) => (
            <RevealItem variant="scale" key={c.slug}>
              ...
              >
                ...
                <span aria-hidden className="pointer-events-none absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-500" style={{ background: "radial-gradient(300px circle at 50% 50%, oklch(0.80 0.075 70 / 0.12), transparent 60%)" }} />
              </Link>
            </RevealItem>
          ))}
        </Reveal>
      ...
      <section id="work" className="container-page py-24 sm:py-32 border-t border-border/40">
        <Reveal variant="clip"><SectionLead eyebrow="Selected Work" title="A small glimpse of recent commissions" /></Reveal>
        <WorkGrid />
      </section>

      {/* ABOUT */}
      <section id="about" className="container-page py-24 sm:py-32 border-t border-border/40 grid lg:grid-cols-2 gap-12 lg:gap-20">
        <Reveal variant="fade-right">
          ...
          <p className="mt-8 text-cream/75 text-lg leading-relaxed">
            Rachnakar Design Studio was founded to bridge two worlds — the soul of Indian craft tradition and
            the exacting tolerances of modern CNC. Every file leaves our studio production-ready, machine-tested
            and tuned for the material it will live in.
          </p>
          <p className="mt-4 text-cream/65 leading-relaxed">
            We work with architects, interior designers, builders, temple committees and homeowners across India.
          </p>
        </Reveal>
        <Reveal stagger={0.1} className="grid grid-cols-2 gap-4 self-start">
          <RevealItem variant="scale"><Stat n="15" label="Catalogue categories" /></RevealItem>
          <RevealItem variant="scale"><Stat n="200+" label="CNC-ready designs" /></RevealItem>
          <RevealItem variant="scale"><Stat n="2000+" label="Projects delivered" /></RevealItem>
          <RevealItem variant="scale"><Stat n="100%" label="Hand-finished" /></RevealItem>
        </Reveal>
      </section>

      {/* CONTACT */}
      <section id="contact" className="container-page py-24 sm:py-32 border-t border-border/40">
        <Reveal variant="perspective"><SectionLead eyebrow="Contact" title="Tell us about your project." /></Reveal>
        <Reveal stagger={0.1} className="mt-14 grid lg:grid-cols-3 gap-8">
          <RevealItem variant="fade-up"><InfoCard label="WhatsApp" value="+91 92844 00646" href={whatsappUrl("Hello Rachnakar Design Studio")} /></RevealItem>
          <RevealItem variant="fade-up"><InfoCard label="Email" value="hello@rachnakar.studio" href="mailto:hello@rachnakar.studio" /></RevealItem>
          <RevealItem variant="fade-up"><InfoCard label="Studio" value="Baner, Pune · Maharashtra, India" href="https://maps.google.com/?q=Baner+Pune" /></RevealItem>
        </Reveal>
      </section>
    </>
  );
...
function SectionLead({ eyebrow, title }: { eyebrow: string; title: string }) {
  return (
    <div className="max-w-3xl">
      <span className="text-[0.7rem] uppercase tracking-[0.42em] text-copper-light/80">{eyebrow}</span>
...
function InfoCard({ label, value, href }: { label: string; value: string; href: string }) {
  return (
    ...
    >
      <span className="text-[0.65rem] uppercase tracking-[0.32em] text-copper-light/80">{label}</span>
    ...
    const ctx = gsap.context(() => {
      ...
      figures.forEach((fig, i) => {
        ...
        tl.fromTo(cover, { yPercent: 0 }, { yPercent: -101, duration: 1.4, delay: i * 0.12 })
          .fromTo(img, { scale: 1.4, filter: "blur(8px)" }, { scale: 1, filter: "blur(0px)", duration: 1.6 }, "<")
      ...
      {works.map((w) => (
        ...
        >
          ...
          <span
            ...
            style={{ background: "radial-gradient(500px circle at 50% 80%, oklch(0.80 0.075 70 / 0.18), transparent 60%)" }}
          />
          <figcaption
            data-work-cap
            className="absolute inset-x-0 bottom-0 p-4 bg-gradient-to-t from-walnut-deep/95 to-transparent"
          >
            <span className="text-[0.6rem] uppercase tracking-[0.28em] text-copper-light/80">{w.tag}</span>
